...
GPU_REQUIRED_MODELS = {"DualTF", "AnomalyTransformer", "TFAD"}
...
class TestExternalBaselineRuntimeSmoke(unittest.TestCase):
    ...
    def test_cpu_compatible_detect_fit_and_detect_score(self):
        self._run_specs(include_gpu_required=False)

    def test_gpu_required_detect_fit_and_detect_score(self):
        if SMOKE_ENV != "catch_env":
            self.skipTest("GPU-required Baselines are only registered in catch_env")
        try:
            import torch
        except ImportError:
            self.skipTest("PyTorch is unavailable")
        if not torch.cuda.is_available():
            self.skipTest("CUDA is unavailable for Baselines with hard-coded CUDA tensors")
        self._run_specs(include_gpu_required=True)
