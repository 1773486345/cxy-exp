...
# Source CATCH official_CATCH.sh: /media/h3c/users/wangyueyang1/cxy/CATCH-master/result/score/CATCH/ASD_dataset_7/run-20260715T150742Z-85080-11778/official_CATCH.sh
# Source CATCH test report: /media/h3c/users/wangyueyang1/cxy/CATCH-master/result/score/CATCH/ASD_dataset_7/run-20260715T150742Z-85080-11778/test_report.1784128749.h3c-R5500-G5.87689.csv
...
SAVE_PATH="${TYPEFUSION_SAVE_PATH:-score/TypeFusion-CATCH/ASD_dataset_7/run-$(date -u +%Y%m%dT%H%M%SZ)-$$}"
...
printf '{"data_name":"ASD_dataset_7.csv","model_name":"typefusion_catch.TypeFusionCATCH","seed":2021,"model_hyper_params":%s}\n' "$MODEL_HYPER_PARAMS" > "$RUN_CONFIG_PATH"
export TYPEFUSION_RUN_CONFIG_PATH="$RUN_CONFIG_PATH"

exec python ./scripts/run_benchmark.py --config-path "unfixed_detect_score_multi_config.json" --data-name-list "ASD_dataset_7.csv" --model-name "typefusion_catch.TypeFusionCATCH" --model-hyper-params "$MODEL_HYPER_PARAMS" --seed 2021 --gpus "$GPU_ID" --num-workers 1 --timeout 60000 --save-path "$SAVE_PATH"
