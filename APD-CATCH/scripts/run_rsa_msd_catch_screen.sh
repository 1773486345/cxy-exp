...
dataset="${1:?usage: run_rsa_msd_catch_screen.sh DATASET}"
python_bin="${PYTHON_BIN:-/media/h3c/users/wangyueyang1/.conda/envs/catch_env/bin/python}"
output_dir="${RSA_MSD_CATCH_OUTPUT_DIR:-result/rsa_msd_catch_screen}"
seed="${RSA_MSD_CATCH_SEED:-2021}"
...
mkdir -p "$output_dir"
PYTHONUNBUFFERED=1 "$python_bin" -c "import json; from ts_benchmark.baselines.rsa_msd_catch.RSAMSDCATCH import run_rsa_msd_catch_screen; run_rsa_msd_catch_screen('$dataset', json.loads('''$params'''), '$output_dir', int('$seed'))"
