...
首次审计时间：2026-07-17 07:18 CST（Git 快照）；进程复查：07:20 CST。阶段一收口复查：2026-07-17 07:32 CST。审计范围为 `cxy/APD-CATCH` 与相邻的 `cxy/CATCH-master`。本次只新增或修改 Markdown 文档；未运行训练、未删除或移动任何结果。
...
### 阶段一收口复查（2026-07-17 07:32 CST）

上方内容是首次审计快照，不是本次收口时的当前 Git 状态。本次从 `cxy/` 仓库根目录重新执行：
...
这表示复查开始时工作树没有已修改、已暂存或未跟踪文件。阶段一收口在 README 第一行加入了更完整的冻结区块；该区块明确 `ts_benchmark/baselines/apd_catch/` 是 legacy exploratory line，现存 v2 命令仅作旧线复现、不应继续运行 15-task screen，新研究以原版 `ts_benchmark/baselines/catch/` 的完整窗口重构协议为基座，且人工确认前没有可运行的新分解模型。

## 运行中的实验

首次审计按脚本名检查时，以下任务均**未运行**：
...
阶段一收口复查时，首次审计中的 MSL score PID 59126 已不在当前列表；主机进程查询显示以下原版 CATCH/benchmark 任务仍在运行：

| PID | 当前命令含义 | 状态 |
| ---: | --- | --- |
| 1011549 | 原版 `catch.CATCH` 的 SMD score 运行 | 运行中 |
| 1366294 | 原版 `catch.CATCH` 的 MSL label 运行 | 运行中 |
| 1989089 | 原版 `catch.CATCH` 的 SWAT label 运行 | 运行中 |

未发现 `run_apd_catch_paper.py`、`run_causal_state_catch_*.sh` 或 `apd_catch` 的运行进程；未终止、暂停或以其他方式改变任何进程。
...
1. README 原先将 v2.0 表述为活动方向，并提供 15-task screen 的并行命令；`CODE_MODIFICATION_LOG.md` 的 “Design Status (2026-07-17)” 也称其为 active Direction A base。该状态与本次复位不一致。阶段一收口时，README 第一行已加入完整冻结区块，并将后续 v2 内容标为 legacy documentation；修改日志保留为历史记录，不改写其当时陈述。
