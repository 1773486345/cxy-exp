...
RUNNER = PROJECT_ROOT / "scripts" / "baselines" / "interfusion_runner.py"
INTERFUSION_PYTHON = (
...
def _as_numeric_frame(data: pd.DataFrame) -> pd.DataFrame:
    ...
    if frame.shape[1] == 0:
        raise ValueError("InterFusion requires at least one numeric feature column.")
    return frame
...
class InterFusionConfig:
    ...
    z_dim: int = 3
    z2_dim: Optional[int] = None
    rnn_hidden: int = 100
    dense_hidden: int = 100
    arnn_hidden: int = 100
    posterior_flow_layers: int = 0
    posterior_flow_type: str = "None"
    pretrain_max_epoch: int = 10
    max_epoch: int = 10
    batch_size: int = 128
...
class InterFusion:
    def __init__(self, **kwargs):
        config_values = InterFusionConfig().__dict__
        config_values.update(kwargs)
        self.config = InterFusionConfig(**config_values)
    ...
    def _run_scores(self, test_data: pd.DataFrame) -> np.ndarray:
        if self.train_values_ is None:
            raise RuntimeError("InterFusion has not been fitted.")
        ...
        if not INTERFUSION_PYTHON.exists():
            raise RuntimeError(f"InterFusion Python env not found: {INTERFUSION_PYTHON}")
        if len(self.train_values_) < int(self.config.window_length):
            raise ValueError("Training series is shorter than InterFusion window_length.")
        if len(test_values) < int(self.config.window_length):
            raise ValueError("Test series is shorter than InterFusion window_length.")

        work_root = PROJECT_ROOT / "result" / "label" / "_baseline_logs" / "interfusion_runs"
        ...
        np.savez(
            ...
            test=test_values,
            label=np.zeros([len(test_values)], dtype=np.float32),
        )

        cmd = [
            str(INTERFUSION_PYTHON),
            str(RUNNER),
            "--input",
            str(input_path),
            "--output",
            ...
            "--dense-hidden",
            str(int(self.config.dense_hidden)),
            "--arnn-hidden",
            str(int(self.config.arnn_hidden)),
            "--posterior-flow-layers",
            str(int(self.config.posterior_flow_layers)),
            "--posterior-flow-type",
            str(self.config.posterior_flow_type),
            "--pretrain-max-epoch",
            str(int(self.config.pretrain_max_epoch)),
            "--max-epoch",
        ...
        if self.config.z2_dim is not None:
            cmd.extend(["--z2-dim", str(int(self.config.z2_dim))])
        ...
        if completed.returncode != 0:
            raise RuntimeError(f"InterFusion runner failed with code {completed.returncode}")
    ...
    def detect_label(self, test_data: pd.DataFrame):
        ...
        if self.train_scores_ is None:
            raise RuntimeError("InterFusion train scores are unavailable.")
    ...
    def cleanup_cached_runs(self) -> None:
        work_root = PROJECT_ROOT / "result" / "label" / "_baseline_logs" / "interfusion_runs"
        if work_root.exists():
