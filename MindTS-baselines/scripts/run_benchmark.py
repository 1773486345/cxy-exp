...
def _preseed_cuda_visible_devices(argv):
    if "--gpus" not in argv:
        return
    gpu_index = argv.index("--gpus")
    gpu_devices = []
    for value in argv[gpu_index + 1:]:
        if value.startswith("--"):
            break
...
_preseed_cuda_visible_devices(sys.argv)
...
def build_data_config(args: argparse.Namespace, config_data: Dict) -> Dict:
    ...
    if args.text_name_list is not None:
        data_config["text_name_list"] = args.text_name_list
    if args.is_mmd is not None:
        is_mmd = args.is_mmd[0] if isinstance(args.is_mmd, list) else args.is_mmd
        data_config["is_mmd"] = bool(is_mmd)
    return data_config
...
def validate_cuda_request(args: argparse.Namespace) -> NoReturn:
    if not args.gpus:
        return
    if torch.cuda.is_available():
        return
    raise RuntimeError(
        "GPU devices were requested with --gpus "
        f"{args.gpus}, but PyTorch cannot see any CUDA device. "
        f"CUDA_VISIBLE_DEVICES={os.environ.get('CUDA_VISIBLE_DEVICES')}, "
        f"torch.version.cuda={torch.version.cuda}, "
        f"torch.cuda.device_count()={torch.cuda.device_count()}. "
        "Check that the job/session has GPU device access and that the requested "
        "GPU ids exist on this server."
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=None,
        help="List of text names entered by the user",
    )
