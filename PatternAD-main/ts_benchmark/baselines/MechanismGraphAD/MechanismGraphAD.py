"""MechanismGraphAD: dynamic relation-state conditional anomaly detection.
...
interface directly.  It does not depend on the closed A2/A3/B routes.
"""
...
DEFAULT_MECHANISM_GRAPH_AD_HYPER_PARAMS = {
    "seq_len": 96,
    "d_model": 64,
    "graph_dim": 32,
    "d_ff": 128,
...
@dataclass
class MechanismGraphADConfig:
    """Validated configuration for a single MechanismGraphAD run."""
    ...
    @classmethod
    def from_kwargs(cls, kwargs: Mapping[str, object]) -> "MechanismGraphADConfig":
        values = dict(DEFAULT_MECHANISM_GRAPH_AD_HYPER_PARAMS)
...
class MultiScaleTemporalEncoder(nn.Module):
    """Shared variable-wise temporal backbone with short, medium, and long views."""

    def __init__(self, n_features: int, config: MechanismGraphADConfig):
        super().__init__()
...
class DirectedRelationLayer(nn.Module):
    ...
    def forward(self, history: torch.Tensor, scale_tokens: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        if dimensions < 2:
            raise ValueError("MechanismGraphAD requires at least two variables.")
...
class MechanismGraphADNet(nn.Module):
    ...
    def __init__(self, n_features: int, config: MechanismGraphADConfig):
        ...
        self.n_features = int(n_features)
        self.config = config
        self.encoder = MultiScaleTemporalEncoder(self.n_features, config)
...
class MechanismGraphAD:
    """Paper-oriented multivariate detector with a normal-only training protocol."""

    def __init__(self, **kwargs):
        self.config = MechanismGraphADConfig.from_kwargs(kwargs)
        requested_device = self.config.device
        if requested_device is None:
            requested_device = "cuda" if torch.cuda.is_available() else "cpu"
        self.device = torch.device(requested_device)
        if self.device.type == "cuda" and not torch.cuda.is_available():
            raise RuntimeError("MechanismGraphAD was configured for CUDA, but CUDA is unavailable.")
        self.scaler = StandardScaler()
        self.model: Optional[MechanismGraphADNet] = None
    ...
    def detect_hyper_param_tune(self, train_data: pd.DataFrame) -> None:
        if train_data.shape[1] < 2:
            raise ValueError("MechanismGraphAD requires at least two observed variables.")
    ...
    def detect_fit(self, train_data: pd.DataFrame, train_label: Optional[pd.DataFrame] = None) -> "MechanismGraphAD":
        del train_label
        ...
        self.model = MechanismGraphADNet(int(self.config.n_features), self.config).to(self.device)
    ...
    def get_diagnostics(self) -> Dict[str, object]:
        return copy.deepcopy(self.fit_diagnostics_ or {"model": "MechanismGraphAD", "status": "unfitted"})
