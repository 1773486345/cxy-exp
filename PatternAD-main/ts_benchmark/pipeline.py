...
from ts_benchmark.evaluation.evaluate_model import eval_model, eval_multi_model, eval_mmd_model
...
def pipeline(
    ...
    text_name_list = data_config.get("text_name_list", None)
    is_mmd = data_config.get("is_mmd", None)
    ...
    data_name_list = list(dict.fromkeys(data_name_list))

    if not data_name_list:
        raise ValueError("No dataset specified.")

    if text_name_list:
        if len(text_name_list) != len(data_name_list):
            raise ValueError(
                "data_name_list and text_name_list must have the same length "
                "for multimodal evaluation."
            )
        data_src.load_multi_list(data_name_list, text_name_list)
    ...
    if is_mmd:
        result_list = [
            eval_mmd_model(model_factory, data_name_list, text_name_list, evaluation_config)
            for model_factory in model_factory_list
        ]

    elif text_name_list:
        result_list = [
            eval_multi_model(model_factory, data_name_list, text_name_list, evaluation_config)
            for model_factory in model_factory_list
        ]
