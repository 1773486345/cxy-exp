"""Targetwise wrapper for B2c's family-wise, per-bin ECRC."""
...
from scripts.multi_evidence.reliability_calibration import SCORE_COMPONENTS


class MultiTargetFamilyWiseEvidenceReliabilityCalibration:
    """Own one B2c calibration object for every target without flattening."""
    ...
    def __init__(
        ...
        self.component_alphas = dict(
            DEFAULT_COMPONENT_ALPHAS if component_alphas is None else component_alphas
        )
        self.mode = str(mode)
        ...
        self.calibrators: Dict[int, FamilyWiseEvidenceReliabilityCalibration] = {}
    ...
    def _validate_matrix_scores(
        ...
        for component in SCORE_COMPONENTS:
            values = np.asarray(scores.get(component), dtype=np.float64)
            if values.shape != (expected_rows, self.target_count):
                raise ValueError(
    ...
    def fit(
        ...
        for position, target_index in enumerate(self.target_indices):
            self.calibrators[target_index] = FamilyWiseEvidenceReliabilityCalibration(
                ...
                min_outer_per_stratum=self.min_outer_per_stratum,
            ).fit(
    ...
    def _require_fitted(self) -> None:
        if set(self.calibrators) != set(self.target_indices):
            raise RuntimeError("Multi-target B2c calibration is not fitted.")
    ...
    def transform(
        ...
        result = {name: np.asarray(values, dtype=np.float64).copy() for name, values in raw_scores.items()}
        ...
        result.update(
            {name: np.stack(columns, axis=1) for name, columns in transformed_columns.items()}
        )
        return result
    ...
    def exceeds(self, scored: Mapping[str, np.ndarray], tail_component: str) -> np.ndarray:
        ...
        strata = np.asarray(scored[f"{component}_reliability_stratum"], dtype=np.int64)
        if values.shape != strata.shape or values.shape[1] != self.target_count:
            raise ValueError("B2c tail and stratum matrices have incompatible shapes.")
            ...
            threshold = self.calibrators[target_index].threshold_array(
                {
                    ...
                    f"{component}_reliability_stratum": strata[:, position],
                },
                tail_component,
            )
    ...
    def metadata(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "calibration_by_target": {
                str(target): self.calibrators[target].metadata() for target in self.target_indices
            },
        }
