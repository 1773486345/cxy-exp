#!/usr/bin/env python3
"""Run B2a-GC without changing B1 repair heads or ECRC semantics."""
...
def run_experiment(
    ...
    """Train B1-style targetwise repair heads and evaluate B2a-GC gates."""
    if output_dir.exists():
        raise FileExistsError(f"Refusing to overwrite B2a-GC output directory: {output_dir}")
    ...
    _write_json_atomic(output_dir / "b2a_gc_evaluation.json", result)
    _write_json_atomic(
        output_dir / "run_metadata.json",
        {
            ...
            "outputs": {
                "evaluation": "b2a_gc_evaluation.json",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print(f"B2a-GC status: {result['status']}; results: {output_dir}")
