#!/usr/bin/env python3
"""Run A2-M4's frozen confirmation after its event-pre control passes."""
...
from scripts.a2.generate_transition_contract import _load_json, generate_suite  # noqa: E402
from scripts.a2.run_m1b_confirmation import _seed_pairs  # noqa: E402
from scripts.a2.run_transition_compatibility import _canonical_hash, run_experiment, write_result  # noqa: E402
from ts_benchmark.baselines.A2TransitionCompatibility import A2LandmarkCompatibility  # noqa: E402


DEFAULT_CONTRACT_CONFIG = REPO_ROOT / "config" / "a2" / "transition_contract_v2.json"
DEFAULT_CONFIRMATION_CONFIG = REPO_ROOT / "config" / "a2" / "m4_confirmation_v1.json"
DEFAULT_M4_CONFIG = REPO_ROOT / "config" / "a2" / "landmark_direction_m4_v1.json"
...
def _require_passing_ablation(ablation_path: Path, base_contract: Mapping[str, Any]) -> None:
    ...
    if not bool(ablation.get("gate", {}).get("passed")):
        raise RuntimeError("A2-M4 confirmation requires a passing event-pre ablation artifact.")
    expected_contract_hash = _canonical_hash(base_contract)
    if str(ablation.get("contract_config_hash")) != expected_contract_hash:
        raise RuntimeError(
            "A2-M4 ablation artifact was generated from a different contract configuration."
        )
...
def _preflight_contract_seeds(
    ...
    if failed:
        raise RuntimeError(f"A2-M4 confirmation contract preflight failed: {failed}")
    return audits


def _development_contract(
    base_contract: Mapping[str, Any], confirmation: Mapping[str, Any]
) -> Dict[str, Any]:
    if "development_contract_seed" not in confirmation:
        raise ValueError("A2-M4 confirmation requires development_contract_seed.")
    return dict(base_contract, seed=int(confirmation["development_contract_seed"]))


def main(argv: List[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--contract-config", type=Path, default=DEFAULT_CONTRACT_CONFIG)
    parser.add_argument("--experiment-config", type=Path, default=DEFAULT_M4_CONFIG)
    ...
    _require_passing_ablation(
        arguments.ablation_result, _development_contract(base_contract, confirmation)
    )
    ...
    for contract_seed, model_seed in seed_pairs:
        ...
        summary, checkpoint = run_experiment(
            contract, experiment, model_factory=A2LandmarkCompatibility
        )
    ...
    print(
        f"A2-M4 confirmation complete: {index['passed_runs']}/{index['run_count']} passed "
