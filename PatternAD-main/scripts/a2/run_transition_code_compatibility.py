#!/usr/bin/env python3
"""Run A2-M3's finite transition-code compatibility experiment."""
...
DEFAULT_EXPERIMENT_CONFIG = REPO_ROOT / "config" / "a2" / "transition_code_m3_v1.json"
...
def main(argv: List[str] | None = None) -> int:
    ...
    summary, checkpoint = run_experiment(
        contract_config, experiment_config, model_factory=A2TransitionCodeCompatibility
    )
    write_result(arguments.output_dir, summary, checkpoint)
    print(
        f"A2-M3 complete: gates_passed={summary['all_gates_passed']} "
