#!/usr/bin/env python3
"""Verify A2-M3's frozen event-pre ablation against matched result artifacts."""
...
def analyze_ablation(
    ...
    """Apply the M3 event-pre criterion fixed before the control is observed."""
    for label, summary, expected_condition in (
        ("conditioned", conditioned, True),
        ("unconditional", unconditional, False),
    ):
        if "transition_code" not in str(summary.get("experiment_id", "")):
            raise ValueError(f"{label} result is not an A2-M3 transition-code run.")
        ...
        if summary.get("raw_score_name") != "event_pre_transition_code_surprisal":
            raise ValueError(f"{label} result has the wrong A2-M3 score family.")
        if "transition_code_coverage" not in summary.get("gates", {}):
            raise ValueError(f"{label} result is missing the A2-M3 code-coverage gate.")
    if conditioned.get("contract_config_hash") != unconditional.get("contract_config_hash"):
        raise ValueError("A2-M3 ablation requires the same generated contract configuration.")
    if _experiment_except_condition(conditioned) != _experiment_except_condition(unconditional):
        raise ValueError(
            "A2-M3 ablation configurations may differ only in experiment_id and "
    ...
    unconditional_coverage = bool(unconditional["gates"]["transition_code_coverage"]["passed"])
    unconditional_primary_failed = not bool(unconditional_primary["passed"])
    return {
        "ablation_id": "a2_m3_event_pre_control_v1",
        ...
        "unconditional": {
            ...
            "transition_code_coverage_passed": unconditional_coverage,
        ...
        "gate": {
            ...
            "unconditional_code_coverage_passed": unconditional_coverage,
            "unconditional_primary_gate_failed": unconditional_primary_failed,
            "passed": conditioned_all and unconditional_coverage and unconditional_primary_failed,
        },
    }
...
def main(argv: List[str] | None = None) -> int:
    ...
    print(
        f"A2-M3 event-pre ablation: passed={result['gate']['passed']} "
