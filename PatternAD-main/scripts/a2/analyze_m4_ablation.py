#!/usr/bin/env python3
"""Verify A2-M4's frozen event-pre ablation against matched result artifacts."""
...
def analyze_ablation(
    ...
    """Apply the M4 event-pre criterion fixed before the control is observed."""
    for label, summary, expected_condition in (
        ("conditioned", conditioned, True),
        ("unconditional", unconditional, False),
    ):
        if "landmark_direction" not in str(summary.get("experiment_id", "")):
            raise ValueError(f"{label} result is not an A2-M4 landmark-direction run.")
        ...
        if summary.get("raw_score_name") != "event_pre_landmark_direction_surprisal":
            raise ValueError(f"{label} result has the wrong A2-M4 score family.")
    if conditioned.get("contract_config_hash") != unconditional.get("contract_config_hash"):
        raise ValueError("A2-M4 ablation requires the same generated contract configuration.")
    if _experiment_except_condition(conditioned) != _experiment_except_condition(unconditional):
        raise ValueError(
            "A2-M4 ablation configurations may differ only in experiment_id and "
    ...
    return {
        "ablation_id": "a2_m4_event_pre_control_v1",
...
def main(argv: List[str] | None = None) -> int:
    ...
    print(
        f"A2-M4 event-pre ablation: passed={result['gate']['passed']} "
