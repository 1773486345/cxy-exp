...
def state_tail_diagnostic_summary(
    config: Mapping[str, Any],
    artifact_dir: Path,
    score_dir: Optional[Path],
) -> Optional[Dict[str, Any]]:
    """Evaluate the isolated causal state-tail component when it is exported.

    This is deliberately separate from the primary PatternAD score.  A new
    diagnostic component must establish its own ordering and normal-regime FPR
    contracts before any score combination can be proposed.
    """
    if score_dir is None:
        return None
    component = "causal_delta_state_tail_surprisal"
    first_score_path = score_dir / f"{MECHANISM_ORDER[0]}.npz"
    with np.load(first_score_path, allow_pickle=False) as payload:
        if component not in payload.files:
            return None
    summary, mechanism_rows, ordering_rows = evaluate_scores(
        ...
        score_dir,
        component,
        f"{component}_diagnostic",
    )
    same_deviation = [
        row
        for row in ordering_rows
        if row["mechanism"] == "same_deviation_different_context"
    ]
    abrupt_shift = [
        row
        for row in ordering_rows
        if row["mechanism"] == "slow_drift_vs_abrupt_shift"
    ]
    same_correct = int(sum(bool(row["correct"]) for row in same_deviation))
    abrupt_correct = int(sum(bool(row["correct"]) for row in abrupt_shift))
    max_fpr_gap = float(summary["maximum_regime_fpr_gap"])
    return {
        "component": component,
        "score_source": str(score_dir),
        "same_deviation_correct": same_correct,
        "same_deviation_total": len(same_deviation),
        "abrupt_vs_gradual_correct": abrupt_correct,
        "abrupt_vs_gradual_total": len(abrupt_shift),
        "maximum_regime_fpr_gap": max_fpr_gap,
        "advancement_gate": {
            "same_deviation_required": 3,
            "abrupt_vs_gradual_required": 2,
            "maximum_regime_fpr_gap": 0.05,
            "pass": bool(
                same_correct == 3
                and len(same_deviation) == 3
                and abrupt_correct == 2
                and len(abrupt_shift) == 2
                and max_fpr_gap <= 0.05
            ),
        },
        "mechanisms": mechanism_rows,
        "matched_orderings": ordering_rows,
...
def main() -> int:
    ...
    state_tail_summary = state_tail_diagnostic_summary(
        config, artifact_dir, score_dir
    )
    if state_tail_summary is not None:
        summary["causal_delta_state_tail_diagnostic"] = state_tail_summary
