...
class PatternADCoreTest(unittest.TestCase):
    ...
    def test_causal_delta_state_ignores_current_and_future_values(self):
        model = JointMultivariateReconstructor(
            _small_config(
                reconstruction_distribution="gaussian",
                use_causal_delta_innovation_diagnostics=True,
        ...
        changed = x.clone()
        changed[:, 4:, :] += 1000.0
        first = model(x, return_causal_state=True)
        second = model(changed, return_causal_state=True)
        self.assertIn("causal_delta_state", first)
        torch.testing.assert_close(
            first["causal_delta_state"][:, :4, :],
            second["causal_delta_state"][:, :4, :],
        )

    def test_returning_causal_state_does_not_change_reconstruction_outputs(self):
        model = JointMultivariateReconstructor(
            _small_config(
                reconstruction_distribution="gaussian",
                use_causal_delta_innovation_diagnostics=True,
        ...
        x = torch.randn(2, 6, 2)
        standard = model(x)
        with_state = model(x, return_causal_state=True)
        for name in (
            "mean",
            "scale",
            "df",
            ...
            torch.testing.assert_close(standard[name], with_state[name])

    def test_state_conditioned_tail_is_state_specific_and_monotonic(self):
        scorer = PatternAwareScorer(
            distribution="gaussian",
            state_tail_clusters=2,
            state_tail_min_cell_size=1,
            state_tail_shrinkage=0.0,
            state_tail_seed=7,
        )
        reference_true = np.array([[[0.0], [0.1], [1.5], [2.0], [3.0]]])
        reference_pred = np.zeros_like(reference_true)
        reference_scale = np.ones_like(reference_true)
        reference_state = np.array([[[0.0], [-1.0], [-1.0], [1.0], [1.0]]])
        scorer.fit_state_conditioned_tail(
            reference_true,
            reference_pred,
            {"scale": reference_scale},
            reference_state,
        )
        query_true = np.array([[[0.0], [1.0], [2.0], [1.0], [1.0]]])
        query_state = np.array([[[0.0], [-1.0], [-1.0], [1.0], [1.0]]])
        scores = scorer.score_state_conditioned_tail(
            query_true,
            np.zeros_like(query_true),
            {"scale": np.ones_like(query_true)},
            query_state,
        )
        self.assertEqual(scores.shape, query_true.shape)
        self.assertGreater(scores[0, 2, 0], scores[0, 1, 0])
        self.assertGreater(scores[0, 1, 0], scores[0, 3, 0])
        summary = scorer.state_conditioned_calibration_summary()
        self.assertTrue(summary["excluded_initial_time_per_window"])
        self.assertEqual(summary["reference_state_count"], 4)
    ...
    def test_causal_delta_state_tail_requires_delta_training(self):
        with self.assertRaisesRegex(ValueError, "require a trained causal delta loss"):
            PatternADConfig(
                reconstruction_distribution="gaussian",
                use_causal_delta_state_tail_diagnostics=True,
            )
        config = PatternADConfig(
            reconstruction_distribution="gaussian",
            reconstruction_causal_delta_innovation_loss_weight=1.0,
        ...
        self.assertTrue(config.use_causal_delta_innovation_diagnostics)
        self.assertTrue(config.use_causal_delta_state_tail_diagnostics)
