...
DEFAULT_PATTERN_AD_HYPER_PARAMS = {
    ...
    "use_causal_delta_state_tail_diagnostics": False,
    ...
    "causal_delta_state_tail_clusters": 4,
    "causal_delta_state_tail_min_cell_size": 128,
    "causal_delta_state_tail_shrinkage": 128.0,
    "causal_delta_state_tail_max_states": 50000,
    "causal_delta_state_tail_seed": 2021,
...
class PatternADConfig:
    def __init__(self, **kwargs):
        ...
        self.reconstruction_causal_innovation_loss_weight = max(
            float(
                getattr(
                    self,
        ...
        self.use_causal_delta_state_tail_diagnostics = bool(
            getattr(self, "use_causal_delta_state_tail_diagnostics", False)
        )
        if self.use_causal_delta_state_tail_diagnostics:
            self.use_causal_delta_innovation_diagnostics = True
        ...
        if (
            ...
            raise ValueError(
                "Causal innovation training requires gaussian or student_t reconstruction."
            )
        if (
            self.use_causal_delta_state_tail_diagnostics
            and self.reconstruction_causal_delta_innovation_loss_weight <= 0.0
        ):
            raise ValueError(
                "Causal delta state-tail diagnostics require a trained causal delta loss."
            )
        ...
        self.causal_delta_state_tail_clusters = max(
            1, int(getattr(self, "causal_delta_state_tail_clusters", 4))
        )
        self.causal_delta_state_tail_min_cell_size = max(
            1,
            int(getattr(self, "causal_delta_state_tail_min_cell_size", 128)),
        )
        self.causal_delta_state_tail_shrinkage = max(
            0.0,
            float(getattr(self, "causal_delta_state_tail_shrinkage", 128.0)),
        )
        self.causal_delta_state_tail_max_states = max(
            2,
            int(getattr(self, "causal_delta_state_tail_max_states", 50000)),
        )
        self.causal_delta_state_tail_seed = int(
            getattr(self, "causal_delta_state_tail_seed", 2021)
        )
...
class JointMultivariateReconstructor(nn.Module):
    ...
    def __init__(self, config):
        ...
        # This branch is deliberately created after the reconstruction backbone
        # so enabling its diagnostics does not perturb the initialization of the
        # established conditional-reconstruction path under a fixed seed.
        self.causal_innovation_encoder = nn.GRU(
            input_size=self.enc_in,
            hidden_size=config.d_model,
            batch_first=True,
        )
        self.use_causal_innovation_diagnostics = bool(
            getattr(config, "use_causal_innovation_diagnostics", False)
        )
        self.causal_innovation_output_proj = nn.Sequential(
            nn.Linear(config.d_model, config.d_ff),
            _activation_module(config.activation),
            nn.Dropout(config.dropout),
    ...
    def _init_causal_innovation_bias(self, config):
        output_layer = self.causal_innovation_output_proj[-1]
        init_scale = float(getattr(config, "distribution_init_scale", 1.0))
        scale_offset = max(init_scale - self.distribution_min_scale, 1e-6)
        scale_bias = (
            scale_offset
            if scale_offset > 20.0
            else math.log(math.expm1(scale_offset))
        )
        with torch.no_grad():
            output_layer.weight[self.enc_in:, :].zero_()
    ...
    def forward(self, x, mask=None, return_causal_state=False):
        if x.ndim != 3:
        ...
        if return_causal_state:
            # This state is strictly h_t(x_{<t}); keep it out of the normal
            # reconstruction/mask path because its feature width is not D.
            result["causal_delta_state"] = causal_hidden
...
class PatternAD:
    ...
    def __init__(self, **kwargs):
        ...
        self.causal_delta_state_scorer = None
        self.model = None
        ...
        self._causal_delta_state_scorer_reference_diagnostics = None
    ...
    def get_diagnostics(self):
        ...
        if self.causal_delta_state_scorer is not None:
            state_calibration = (
                self.causal_delta_state_scorer.state_conditioned_calibration_summary()
            )
            if state_calibration is not None:
                state_calibration.update(
                    copy.deepcopy(
                        self._causal_delta_state_scorer_reference_diagnostics or {}
                    )
                )
            diagnostics["causal_delta_state_score_calibration"] = state_calibration
        return diagnostics
    ...
    def _build_causal_delta_state_scorer(self):
        """Normal-only tail map keyed by the learned causal history state."""
        return PatternAwareScorer(
            components=("raw",),
            eps=getattr(self.config, "pattern_score_eps", 1e-6),
            score_mode="contextual_tail_probability",
            distribution="gaussian",
            state_tail_clusters=getattr(
                self.config, "causal_delta_state_tail_clusters", 4
            ),
            state_tail_min_cell_size=getattr(
                self.config, "causal_delta_state_tail_min_cell_size", 128
            ),
            state_tail_shrinkage=getattr(
                self.config, "causal_delta_state_tail_shrinkage", 128.0
            ),
            state_tail_max_states=getattr(
                self.config, "causal_delta_state_tail_max_states", 50000
            ),
            state_tail_seed=getattr(
                self.config, "causal_delta_state_tail_seed", 2021
            ),
        )
    ...
    def _fit_causal_delta_scorer(self, data_loader):
        ...
        self._causal_delta_state_scorer_reference_diagnostics = None
        fit_scale_tail = bool(
            self.config.use_causal_delta_contextual_tail_diagnostics
        )
        fit_state_tail = bool(
            self.config.use_causal_delta_state_tail_diagnostics
        )
        if not (fit_scale_tail or fit_state_tail):
            return
        ...
        self.model.eval()
        true_windows, mean_windows, scale_windows, state_windows = [], [], [], []
            ...
            for batch_x_time, _, _, _ in data_loader:
                ...
                outputs = self._align_output_to_true(
                    self._forward_backbone(
                        batch_x_time,
                        None,
                        return_causal_state=fit_state_tail,
                    ),
                    batch_x_time,
                )
                ...
                if fit_state_tail:
                    if "causal_delta_state" not in outputs:
                        raise RuntimeError(
                            "Causal delta state-tail calibration requires causal states."
                        )
                    state_windows.append(
                        outputs["causal_delta_state"][:take]
                        .detach()
                        .cpu()
                        .numpy()
                    )
        ...
        reference_true = np.concatenate(true_windows, axis=0)
        reference_mean = np.concatenate(mean_windows, axis=0)
        reference_scale = np.concatenate(scale_windows, axis=0)
        reference_metadata = {
        ...
        if fit_scale_tail:
            ...
            self._causal_delta_scorer_reference_diagnostics = {
                **reference_metadata,
                "cells": int(reference_true.size),
                "bin_counts": summary.get("bin_counts", []),
            }
        if fit_state_tail:
            if not state_windows:
                raise RuntimeError("No causal states collected for delta state-tail calibration.")
            self.causal_delta_state_scorer = self._build_causal_delta_state_scorer()
            self.causal_delta_state_scorer.fit_state_conditioned_tail(
                reference_true,
                reference_mean,
                {"scale": reference_scale},
                np.concatenate(state_windows, axis=0),
            )
            summary = (
                self.causal_delta_state_scorer.state_conditioned_calibration_summary()
                or {}
            )
            self._causal_delta_state_scorer_reference_diagnostics = {
                **reference_metadata,
                "cells": int(summary.get("reference_cell_count", 0)),
                "states": int(summary.get("reference_state_count", 0)),
        ...
        if isinstance(outputs, dict):
            return {
                name: (
                    ...
                    else PatternAD._align_output_to_true(value, true)
                )
    ...
    def _forward_backbone(self, batch_x_time, mask=None, return_causal_state=False):
        if return_causal_state:
            return self.model(
                batch_x_time,
                mask,
                return_causal_state=True,
            )
    ...
    def _reconstruction_loss(self, outputs, target, masked_positions):
        ...
        causal_weight = self.config.reconstruction_causal_innovation_loss_weight
        if causal_weight > 0.0 and target.shape[1] > 1:
            if not (
                isinstance(outputs, dict)
                and "causal_innovation_mean" in outputs
                and "causal_innovation_scale" in outputs
            ):
                raise ValueError(
                    "Causal innovation loss requires causal mean and scale outputs."
                )
            causal_mean = outputs["causal_innovation_mean"][:, 1:, :]
            causal_scale = outputs["causal_innovation_scale"][:, 1:, :].clamp_min(
                self.config.distribution_min_scale
            )
            causal_standardized = (target[:, 1:, :] - causal_mean) / causal_scale
            causal_loss = (
                0.5 * causal_standardized.square()
                + torch.log(causal_scale)
                + 0.5 * math.log(2.0 * math.pi)
            ).mean()
    ...
    def _collect_multi_scores(self, data_loader, total_len: int) -> np.ndarray:
        ...
        window_components = {
            ...
            "causal_delta_state_tail_surprisal": [],
        }
        batch_count = 0
        window_count = 0
        scale_count = 0
        ...
        with torch.no_grad():
            for batch_x_time, _, _, _ in data_loader:
                ...
                causal_delta_state_tail = torch.zeros_like(raw_squared)
                if (
                    isinstance(outputs, dict)
                    and (
                    ...
                    causal_outputs = self._align_output_to_true(
                        self._forward_backbone(
                            ...
                            None,
                            return_causal_state=(
                                self.causal_delta_state_scorer is not None
                            ),
                        ),
                        batch_x_time,
                    )
                    ...
                    if "causal_delta_innovation_mean" in outputs:
                        ...
                        if self.causal_delta_state_scorer is not None:
                            if "causal_delta_state" not in causal_outputs:
                                raise RuntimeError(
                                    "Causal delta state-tail scoring requires causal states."
                                )
                            tail = self.causal_delta_state_scorer.score_state_conditioned_tail(
                                causal_delta_target,
                                causal_delta_mean,
                                distribution_params={
                                    "scale": causal_delta_innovation_scale
                                },
                                state=causal_outputs["causal_delta_state"],
                            )
                            causal_delta_state_tail = torch.as_tensor(
                                tail,
                                dtype=batch_x_time.dtype,
                                device=batch_x_time.device,
                            )
                ...
                component_values = {
                    ...
                    "causal_innovation_squared_residual": causal_innovation_squared,
                    "causal_innovation_standardized_squared_residual": (
                        causal_innovation_squared
                    ...
                    "causal_delta_state_tail_surprisal": causal_delta_state_tail,
                }
                ...
                if score_mask is None:
                    ...
                    aggregated_components = {
                        name: (
                            values.mean(dim=-1)
                            if name in causal_component_names
                            else (values * component_mask).sum(dim=-1)
                            / component_count
                        )
                        for name, values in component_values.items()
                    }
