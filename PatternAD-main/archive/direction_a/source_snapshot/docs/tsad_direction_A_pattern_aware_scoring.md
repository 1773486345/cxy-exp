...
### 2026-07-12 status: direction A residual-semantics line is closed

结论先行：当前方向 A 的“用单一条件残差/尾概率分数同时解释波动背景和
突变 onset”的主线失败，停止继续调参、分数融合、多 seed 扩展、P2、locked
confirmation 和真实数据实验。这个结论来自已冻结的机制门槛，而不是因为某个
训练任务未完成。

已完成的证据链如下。

1. `P1-v2-holdout` 的 10 generator seeds x 3 model seeds x 4 A cells 未通过
   两个主 gate：`A11-A01` matched-ordering 仅 `+0.00667`（95% CI
   `[-0.100, 0.100]`），maximum regime-FPR gap 相对 A00 仅下降 `5.40%`。
2. past-only level innovation 的 same-deviation 为 `0/3`、abrupt/gradual 为
   `1/2`；它不能作为可靠 change detector。
3. past-only delta innovation 修复了 abrupt/gradual（`2/2`），但
   same-deviation 为 `0/3`。以 predicted delta scale 分层的独立 normal ECDF
   `causal_delta_contextual_tail_surprisal` 保留 abrupt/gradual `2/2`，仍为
   same-deviation `0/3`。
4. 为避免把“scale 不是充分状态”误判为整个问题失败，最后一次、机制上不同的
   诊断复用了 causal GRU 的 `h_t(x_{<t})`，在独立 normal score-reference 上
   以 `state cluster x variable` 经验 tail CDF 评分，导出
   `causal_delta_state_tail_surprisal`。它不改主分数、不读 `x_t` 或未来值，
   也不使用 calibration/test/label。固定 generator `3101`、model seed `2021`
   和 30-epoch budget 的运行早停于 epoch 5；reference 有 2,116 个 causal
   states、10,580 个 post-initial cells。结果为 quiet/volatile `1/3`、
   abrupt/gradual `0/2`，尽管 maximum normal-regime FPR gap 是合格的
   `0.01873`（阈值 `0.05`）。

因此，现有证据不支持“一个可校准的 scalar residual surprise 能在这两个反事实
任务中同时承载运行状态语义与突变语义”的论文主张。这里不把它包装成暂时的
negative result：方向 A 的本项目主线到此关闭。后续如继续研究，只能作为与本
主张分离的新问题，例如事件级 transition mechanism、context OOD 的独立分数，
或直接转向方向 B；不得把这些结果叙述为当前 PatternAD 的有效增益。
...
P1-v2-holdout 随后在独立 normal score-reference segment 的分层经验尾概率下重新完成全部 120 个 cell。run provenance 与当前代码一致。A11 相对 A00 的 macro AP 仍提升 `0.03494`，95% CI `[0.02628, 0.04437]`；然而 P1 的两个核心 gate 仍失败：A11-A01 总 matched-ordering 仅 `+0.00667`，95% CI `[-0.100, 0.100]`，而 maximum regime-FPR gap 的相对下降仅 `5.40%`，95% CI 覆盖负值，低于预注册的 `25%`。分解表明 C1/D1 将 same-deviation ordering 提升 `+0.4556`，却将 slow-drift vs abrupt ordering 降低 `-0.6667`。这说明问题不是经验尾参考数据复用，而是双向条件重构能够借助突变后的状态解释突变本身。

因此 P1-v3 不再重试双向 transition-scale 或手工 gate。新增的开发诊断是严格因果 innovation head：对每个 `t`，它只读取 `x_{<t}`，以 Gaussian innovation NLL 训练，`x_t` 与未来值均不进入该 head。它暂不加入最终异常分数；先独立检查其 standardized innovation 是否在 abrupt/gradual ordering 上转正，同时不破坏 same-deviation 的 level-tail 证据。只有该诊断在预声明的多 seed 合成机制中成立，才冻结与 level score 的 p-value 组合规则。
