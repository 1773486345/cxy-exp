...
is now disabled in all formal cells. P1-v1 was superseded by P1-v2 and its raw
development cells were purged after Direction A was closed; the metrics above
...
meet the matched-ordering or regime-FPR gates, so P2 is closed. The causal level
innovation (`0/3` quiet/volatile, `1/2` abrupt/gradual), causal delta innovation
(`0/3`, `2/2`), and scale-stratified delta tail (`0/3`, `2/2`) diagnostics are
also complete and cannot be combined.

The final state-conditioned delta-tail diagnostic was run once with this
command:
...
conda run --no-capture-output -n patternad_env \
  ...
  --hyperparameter-overrides '{"reconstruction_causal_delta_innovation_loss_weight": 1.0, "use_causal_delta_state_tail_diagnostics": true}' \
  --method-name A11_causal_delta_state_tail_dev \
  --output-dir result/patternad_synthetic/dev_causal_delta_state_tail
```

It uses only the learned `h_t(x_{<t})` causal GRU state to select a
normal-reference `state cluster x variable` empirical tail, and exports
`causal_delta_state_tail_surprisal`. The complete result is in
`result/patternad_synthetic/dev_causal_delta_state_tail/`: quiet/volatile is
`1/3`, abrupt/gradual is `0/2`, and FPR gap is `0.01873`. Its frozen gate was
`3/3`, `2/2`, and FPR gap `<=0.05`, so it failed. Do not tune its cluster count,
shrinkage, projection, quantile head, or fusion; do not start multi-seed,
confirmation, P2, or real-data experiments for Direction A.
