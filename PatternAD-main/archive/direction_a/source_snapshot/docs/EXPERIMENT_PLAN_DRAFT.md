...
Difference-space 诊断已完成：`causal_delta_innovation_standardized_squared_residual` 的 abrupt/gradual 为 `2/2`（margin `+4.2737`、`+3.2558`），但 same-deviation 为 `0/3`；因果尺度在 volatile context 较大，却未使标准化残差的 normal tail 跨 regime 校准。随后按 predicted causal-delta scale 分层、仅在独立 normal score-reference 拟合的 `causal_delta_contextual_tail_surprisal` 保留 abrupt/gradual `2/2`，但 same-deviation 仍为 `0/3`，未通过其扩展门槛。

作为最后一次、机制上不同而非继续调 scale-bin 的开发诊断，`causal_delta_state_tail_surprisal` 使用同一 causal delta GRU 的 strictly-past state `h_t(x_{<t})` 聚为 4 个 normal-reference state clusters，再对每个 `(cluster, variable)` 拟合收缩经验 tail CDF。它不进入 primary score；target/future perturbation、reference-only fit、t=0 exclusion、小 cell fallback 和 primary reconstruction isolation 均有单元测试。固定 `generator=3101`、`model seed=2021`、30-epoch budget 的运行早停于 epoch 5，记录在 `result/patternad_synthetic/dev_causal_delta_state_tail/`。该分量的 quiet/volatile 为 `1/3`、abrupt/gradual 为 `0/2`，maximum normal-regime FPR gap 为 `0.01873`。预先冻结的要求是 `3/3`、`2/2`、FPR gap `<=0.05`，因此失败。

此处停止：不得再搜索 cluster 数、shrinkage、state projection、quantile head 或 score fusion；也不得启动多 seed、P2、locked confirmation 或真实数据实验。方向 A 的当前 residual-semantics / conditional-calibration 论文主张关闭，后续只能转为与其分离的新研究问题。

### P2：Motivation development，4 个主 cell，3 seeds（已关闭）

本节保留为历史 protocol，不可执行。上面的最终开发 gate 已失败。
