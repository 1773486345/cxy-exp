...
当前 A2-M1 已按该定义实现为 `P_t` GRU 编码后的全轨迹条件混合似然模型；它不是将 A
退回成 point residual。该模型仍处在合成开发阶段，必须先通过所有 normal-control gate
并在新 generator seed 确认，才能讨论真实数据或和 B 的比较。
